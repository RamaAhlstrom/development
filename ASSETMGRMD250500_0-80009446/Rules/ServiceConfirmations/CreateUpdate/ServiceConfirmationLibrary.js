...
import AutoCreateServiceConfirmation from './AutoCreateServiceConfirmation';
...
export default class ServiceConfirmationLibrary {
    ...
    isAutoConfirmationCreate() {
        return this.getStartPage() === ServiceConfirmationLibrary.autoConfirmationCreate;
    }
        ...
        return this.countOrderConfirmations(context, orderID).then(hasConfirmations => {
            if (hasConfirmations) {
                ...
                if (binding && [
                    ...
                    this.setStartPageFlag(ServiceConfirmationLibrary.autoConfirmationCreate);
                    return AutoCreateServiceConfirmation(context, binding); 
                }
    ...
    createConfirmation(context) {
        let action = context.executeAction({
            ...
            return this.createConfirmationSoldToParty(context);
        }).then(() => {
    ...
    createConfirmationSoldToParty(context) {
        let order = this._confirmation.relatedObjectsData.ServiceOrder || context.binding?.S4ServiceOrder_Nav?.['@odata.readLink'];
        ...
        if (order) {
            ...
            const employeeResponsibleType = '00000014';
            const execServiceEmployeeType = '00000052';
            const payerType = '00000004';

            return context.read('/SAPAssetManager/Services/AssetManager.service', order + '/Partners_Nav' , [], '').then(result => {
                const assignedEmployees = result.filter(partner => {
                    return partner.PartnerFunction === employeeResponsibleType || 
                        partner.PartnerFunction === execServiceEmployeeType;
                });

                const soldToParty = result.filter(partner => {
                    return partner.PartnerFunction === soldToPartyType;
                });

                const billToParty = result.filter(partner => {
                    return partner.PartnerFunction === billToPartyType;
                });

                const payer = result.filter(partner => {
                    return partner.PartnerFunction === payerType;
                });

                let createPartnerActions = [];
                if (soldToParty.length) {
                    const soldToPartyAction = this._getPartnerCreateAction(type, confirmationLink, soldToParty.getItem(0).BusinessPartnerID, soldToPartyType);
                    createPartnerActions.push(soldToPartyAction);
                }

                if (assignedEmployees.length) {
                    const contactAction = this._getPartnerCreateAction(type, confirmationLink, assignedEmployees.getItem(0).BusinessPartnerID, assignedEmployees.getItem(0).PartnerFunction);
                    createPartnerActions.push(contactAction);
                }

                if (billToParty.length) {
                    const billToPartyAction = this._getPartnerCreateAction(type, confirmationLink, billToParty.getItem(0).BusinessPartnerID, billToPartyType);
                    createPartnerActions.push(billToPartyAction);
                }

                if (payer.length) {
                    const payerAction = this._getPartnerCreateAction(type, confirmationLink, payer.getItem(0).BusinessPartnerID, payerType);
                    createPartnerActions.push(payerAction);
                }

                return Promise.all(createPartnerActions.map(action => context.executeAction(action)));
            });
        }
    ...
    _getPartnerCreateAction(confirmationType, confirmationLink, businessPartnerID, partnerFunction) {
        return {
            'Name': '/SAPAssetManager/Actions/ServiceOrders/S4ServiceOrderPartnerCreate.action',
            'Properties': {
                'Target': {
                    'EntitySet': 'S4ServiceConfirmationPartners',
                },
                'Properties': {
                    'ObjectID': '/SAPAssetManager/Rules/ServiceConfirmations/CreateUpdate/Data/GetServiceConfirmationLocalID.js',
...
ServiceConfirmationLibrary.autoConfirmationCreate='Auto_Confirmation_Create';
