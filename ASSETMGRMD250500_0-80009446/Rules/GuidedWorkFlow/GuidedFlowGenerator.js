    ...
    */
    ...
    parseFlows(flows) {
        ...
        if (flows && flows.length === 1) {
            ...
            let featureFlows = this.loopFeatureFlows(flows);
            ...
            identifedFlow = this.identifyFlow(this.loopObjectGroup(flows, identifedFlows, true, true));
            if (!ValidationLibrary.evalIsEmpty(identifedFlow)) {
                return identifedFlow;
            }
            identifedFlow = this.identifyFlow(this.loopObjectGroup(flows, identifedFlows, true, false));
            if (!ValidationLibrary.evalIsEmpty(identifedFlow)) {
                return identifedFlow;
            }
            identifedFlow = this.identifyFlow(this.loopObjectGroup(flows, identifedFlows, false, true));
            if (!ValidationLibrary.evalIsEmpty(identifedFlow)) {
                return identifedFlow;
            }
     ...
     * @returns {ObservableArray} flows
     */
    loopObjectGroup(flows, identifedFlows, ObjectGroupFlag, ObjectGroup1Flag) {
        ...
        return objectGroupIdentifiedFlows.length > 0 ? objectGroupIdentifiedFlows : identifedFlows;
    }
