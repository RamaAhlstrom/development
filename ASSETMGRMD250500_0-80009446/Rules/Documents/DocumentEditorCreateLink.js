...
export default function DocumentEditorCreateLink(context, mediaReadLink) {
    return documentEditorCreateInfo(context).then(info => {
        if (info) {
            ...
            links.push(link);
            promises.push(context.create(serviceName, info.entitySet, properties, links, {'OfflineOData.RemoveAfterUpload':'/SAPAssetManager/Rules/Common/RemoveAfterUploadValue.js'}));
