...
export default class ScannerLibrary {
     ...
     */
    static async init(clientAPI, customScannerParser = undefined) {
        const context = clientAPI.evaluateTargetPathForAPI('#Page:-Current'); 
        ...
        if (context.nativescript?.platformModule?.isAndroid) {
            ...
            if (manufacturer === 'samsung' && scannerType === 'KNOX_CAPTURE') {
                scannerRegistry.setDefaultScanner(scannerRegistry.registerScanner(new KnoxCaptureScanner(context?.currentPage?._context, customScannerParser)));
                ...
                scannerRegistry.setDefaultScanner(scannerRegistry.registerScanner(new ZebraScanner(context?.currentPage?._context, customScannerParser)));
