...
export default class EDTHelper {
    ...
    static getLatestMeasurementDoc(context, point) {
        ...
        if (latestDoc?.['@odata.readLink'] && !latestDoc?.['@sap.hasPendingChanges']) {
            Object.assign(latestDoc, {
                ReadingValue: undefined,
                PrevReadingValue: latestDoc.ReadingValue,
                PrevValuationCode: latestDoc.ValuationCode,
                ValuationCode: undefined,
                ShortText: '',
                '@sap.hasPendingChanges': true,
            });
        }
    ...
    static getOrderObjectNumberFromContext(context) {
        ...
        return binding.WOHeader?.ObjectKey || binding.WOHeader?.OrderMobileStatus_Nav?.ObjectKey || '';
    }
    static getOperationObjectNumberFromContext(context, operationSelected) {
        const binding = context.binding || (context.getActionBinding && context.getActionBinding()) || {};
        ...
        return operation?.ObjectKey || operation?.OperationMobileStatus_Nav?.ObjectKey || '';
    }
}
