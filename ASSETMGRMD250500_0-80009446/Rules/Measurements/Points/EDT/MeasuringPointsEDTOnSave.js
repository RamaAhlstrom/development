...
function validateEDTSection(context, section, sectionIndex, ErrorsHandler) {
    ...
    sectionExtension.getRowBindings().forEach((rowBinding, rowIndex) => {
        ...
        if (!measurementDoc._skipped) {
            if (!shouldSkipEmptyReading(context, measurementDoc)) {
...
function shouldSkipEmptyReading(context, measurementDoc) {
    if (ValidationLibrary.evalIsEmpty(measurementDoc.ValuationCode) && ValidationLibrary.evalIsEmpty(measurementDoc.ReadingValue)) {
