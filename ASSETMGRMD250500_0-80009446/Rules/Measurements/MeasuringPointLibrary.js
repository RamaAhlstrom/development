...
export default class {
    ...
    static loadLocalDefaults(context) {
        ...
        if (readingType === 'MULTIPLE') { //Read local default values for FDC
            ...
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'MeasurementDocuments', ['ReadingValue', 'ValuationCode', 'ShortText'], "$filter=sap.hasPendingChanges() eq true and Point eq '" + point + "'&$orderby=ReadingTimestamp desc&$top=1").then(results => {
    ...
    static measurementDocumentCreateUpdateFieldValues(pageClientAPI) {
        ...
        if (libThis.evalIsUpdateTransaction(pageClientAPI)) {
            if (binding.ReadingValue || (binding.HasReadingValue === 'X' && binding.ReadingValue === 0)) {
                ...
                controls[ReadingSim].setValue(pageClientAPI.formatNumber(binding.ReadingValue, '', {maximumFractionDigits: decimal, minimumFractionDigits : 0}));
            }
    ...
    */
    ...
    /**
     * Evaluates whether we are currently in an create transaction
     */
    static evalIsCreateTransaction(pageClientAPI) {
        return (libCom.getStateVariable(pageClientAPI, 'TransactionType') === 'CREATE');
    }
    ...
    */
    static measuringPointListFieldFormat(sectionProxy) {
        const section = sectionProxy.getParent().getName();
        let property = sectionProxy.getParent().getProperty();
        const binding = sectionProxy.binding;
        let value = '';
                        ...
                        switch (property) {
                            ...
                            case 'Title': {
                                ...
                                if (point.IsCounterReading === 'X') {
                                    const uom = binding.UoM ?? binding.PRTPoint?.UoM;
                                    value = point.CounterReadingDifference;
                                    value = sectionProxy.formatNumber(value, '', { maximumFractionDigits: decimal, minimumFractionDigits : 0 }) + ' ' + uom;
                                }
                                return value;
                            }
                            case 'SubstatusText': {
