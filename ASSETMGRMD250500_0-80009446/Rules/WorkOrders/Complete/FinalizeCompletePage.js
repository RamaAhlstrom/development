...
export default function FinalizeCompletePage(context) {
    ...
    return Promise.all([SupervisorLibrary.checkReviewRequired(context, binding)]).then((checks) => {
...
async function beforeOperationComplete(context, isReviewRequired) {
    ...
    const isLogTimePossible = TimeSheetsIsEnabled(context) || ConfirmationsIsEnabled(context);
    const isTimeStepWasNotSet = !WorkOrderCompletionLibrary.getStepValue(context, 'time');

    if (isNotFinalConfirmation && isConfirmationSupported && isTimeStepWasNotSet) {
        if ((isLogTimePossible || isTimeStepWasNotSet) && !isReviewRequired) {
