 ...
 */
export function evaluateExpression(context, expr) {
    ...
    if (isNaN(Number(expr))) {
        if (parens.test(expr)) {
            // eslint-disable-next-line no-unused-vars
            let newExpr = expr.replace(parens, function(match, subExpr) {
                return evaluateExpression(context, subExpr);
            });
            return evaluateExpression(context, newExpr);
        } else if (exp.test(expr)) {
            // eslint-disable-next-line no-unused-vars
            let newExpr = expr.replace(exp, function(match, base, pow) {
                let result = Math.pow(Number(base), Number(pow));
                return result.toFixed(context.binding.DecimalPlaces);
            });
            return evaluateExpression(context, newExpr);
    ...
    return Number(expr).toFixed(context.binding.DecimalPlaces);
}
