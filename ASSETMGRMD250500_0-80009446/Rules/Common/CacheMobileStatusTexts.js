...
export default function CacheMobileStatusTexts(context) {
    ...
    let readConfigPromises = [
        readConfigsForObjectType(context, objectType),
        readConfigsForObjectType(context, libCom.getGlobalDefinition(context, 'ObjectTypes/Notification.global')),
        readConfigsForObjectType(context, libCom.getGlobalDefinition(context, 'ObjectTypes/Task.global')),
    ];
...
function isCICOTitleOverrideNeeded(context, objectType) {
    ...
    ].includes(objectType) && CommonLibrary.getWorkOrderAssnTypeLevel(context) !== 'SubOperation';
}
