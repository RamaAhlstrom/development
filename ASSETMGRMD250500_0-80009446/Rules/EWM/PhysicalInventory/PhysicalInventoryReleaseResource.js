...
function unassignWHO(context, resource) {
    return context.executeAction({
        'Name': '/SAPAssetManager/Actions/EWM/WarehouseOrders/WarehouseOrderRelease.action', 'Properties': {
            ...
            'Target': {
                ...
                'QueryOptions': `$filter=WarehouseNo eq '${context.binding.WarehouseNo}' and WarehouseOrder eq '${context.binding.WarehouseOrder}'`,
            },
        },
    }).then(() => {
...
function deleteWHData(context) {
    const service = '/SAPAssetManager/Services/AssetManager.service';
    const filter = `$filter=WarehouseNo eq '${context.binding.WarehouseNo}' and WarehouseOrder eq '${context.binding.WarehouseOrder}'`;
    return Promise.all([
