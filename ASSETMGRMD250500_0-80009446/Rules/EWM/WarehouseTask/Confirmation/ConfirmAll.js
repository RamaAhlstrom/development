    ...
    return ValidateItems(clientAPI, selectedItems).then(async (validationResult) => {
        if (!validationResult.allValid) {
            ...
            let actionProperties = {
                'Name': '/SAPAssetManager/Actions/EWM/WarehouseTasks/Confirmation/WarehouseTaskFailure.action',
                'Properties': {
                    'Message': clientAPI.localizeText('wht_missing_information_x', [failedItems.length]),
                },
            };
            clientAPI.executeAction(actionProperties);
            actionProperties = {
