...
export default async function OnWorkOrderChanged(context) {
    ...
    let showCoopButtons = await cooperationIsEnabledForWorkOrder(context, workOrder.PlanningPlant, workOrder.OrderType);
    let coopButtons = libCom.getControlProxy(pageProxy, 'CooperationSeg');
    if (coopButtons && showCoopButtons) { //Show the cooperation buttons if this workorder supports cooperation confirmations
        const captionDescriptor = context.getBindingObject().IsOnCreate ? 'create' : 'update';
        
        coopButtons.setEditable(true);
        if (libCom.getListPickerValue(coopButtons.getValue()) !== 'Support') { //If this is already a cooperation, don't reset the value
            coopButtons.setValue('None');
        } else { //This is a cooperation
            coopButtons.setEditable(false);
        }
        if (captionDescriptor === 'update') { //Edited confirmations cannot be changed to cooperations
            coopButtons.setEditable(false);
        }
    } else { //This workorder does not support cooperation confirmations
        coopButtons.setEditable(false);
        coopButtons.setValue('None');
    }
