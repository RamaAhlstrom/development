...
export default function OnOperationChangeListPickerUpdate(pageProxy) {
    ...
    let cooperation = libCom.getListPickerValue(libCom.getTargetPathValue(pageProxy, '#Control:CooperationSeg/#Value'));
    let enable = !!operation;

    if (cooperation === 'Support') { //Cooperations do not allow editing of activity types or variance reasons
        enable = false;
    }
