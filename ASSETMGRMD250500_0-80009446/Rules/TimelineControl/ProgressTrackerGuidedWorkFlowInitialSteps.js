...
export default async function ProgressTrackerGuidedWorkFlowInitialSteps(context) {
    const objectType = libCommon.getMobileStatusEAMObjectType(context, context.binding);
    const mobileStatusObject = libMobile.getMobileStatusNavLink(context, context.binding);
...
async function mapStatusHistoriesToStepSequences(context, mobileStatusObject, flowId) {
    try {
        ...
        const statusHistories = await context.read(service, `${mobileStatusObject['@odata.readLink']}/PMMobileStatusHistory_Nav`,
            ['MobileStatus'], `$filter=MobileStatus ne '${mobileStatusObject.MobileStatus}'`).then(histories => Array.from(histories).map(history => history.MobileStatus));
    ...
    return context.read(service, 'GuidedFlowStatusSeqs', ['Step', 'FromStatus'], queryOptions)
        .then(guidedFlowStatusSeqs => {
            ...
            for (let statusSeq of guidedFlowStatusSeqs) {
                // Only add steps whose FromStatus and ToStatus are included in history statuses 
                // or which have FromStatus 'RECEIVED', because there isn't a history record for 'RECEIVED'
