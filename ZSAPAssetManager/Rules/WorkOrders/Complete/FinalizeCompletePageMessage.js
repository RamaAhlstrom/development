...
import FinalizeCompletePage from '../../../../SAPAssetManager/Rules/WorkOrders/Complete/FinalizeCompletePage';
...
export default async function FinalizeCompletePageMessage(context) { 
    ...
    let datacompletioncheck=WorkOrderCompletionLibrary;
    let ordertype=context.binding.WOHeader.OrderType;
    let plant=context.binding.WOHeader.PlanningPlant;
    let MaintenanceActivityType=context.binding.WOHeader.MaintenanceActivityType;
    let Manditoryinfo=await CheckManditory(context,ordertype,plant,MaintenanceActivityType);
    let notifiquipfunction
    if(context.binding.NotifNum!="")
    {
        notifiquipfunction=await CheckManditorynotification(context);
    }
    if(Manditoryinfo!=undefined)
    {
        //"ZTECHOBJ":"X","ZMSCAN":"X"
        let OperationFunctionLocation=context.getPageProxy().binding.OperationFunctionLocation;
        if(OperationFunctionLocation==null)
        {
            OperationFunctionLocation="";
        }
        let OperationEquipment=context.getPageProxy().binding.OperationEquipment;
        if(OperationEquipment==null)
        {
            OperationEquipment="";
        }
        if(OperationFunctionLocation=="" && OperationEquipment=="")
        {
            if(context.binding.NotifNum!="")
            {
                OperationFunctionLocation=notifiquipfunction.HeaderFunctionLocation;
                OperationEquipment=notifiquipfunction.HeaderEquipment;
            }
        }
      
        if(Manditoryinfo.ZTECHOBJ=="X")
        {
            if (OperationFunctionLocation != "" || OperationEquipment != "") {
                ...
                return context.executeAction(
                    {
                        ...
                        'Properties': {
                            'Title': context.localizeText("completion_WO_title"),
                            'Message': context.localizeText("Operation_Technical_Object_Manditory"),//"Operation Technical Object Manditory",//context.localizeText(message)
        ...
        if(Manditoryinfo.ZMSCAN=="X")
        {
            //if (context.getGlobalDefinition('/SAPAssetManager/Globals/WorkOrder/ZOperationTechObjectupdateStatus.global').data.Value=="Gowri") {
            //if (context.binding.Zstatus=="") {
                ...
                return context.executeAction(
                    {
                        ...
                        'Properties': {
                            'Title': context.localizeText("completion_WO_title"),
                            'Message': context.localizeText("Please_update_Technical_Object"),//"Please update Technical Object",//context.localizeText(message)
                            'OKCaption': context.localizeText('ok'),
                            'OnOK': '',
                        },
                    },
    ...
    {
    if(context.getPageProxy().binding.OperationCategory=="MAIN")
        {
            if(context.getPageProxy().binding.NotifNum=="" && context.getPageProxy().binding.WOHeader.NotificationNumber==""){
                return context.executeAction(
                    {
                        'Name': '/SAPAssetManager/Actions/Common/GenericWarningDialog.action',
                        'Properties': {
                            'Title': context.localizeText("completion_WO_title"),
                            'Message': context.localizeText("No_notification_is_assigned_to_maintance_order"),//"No notification is assigned to maintance order",//context.localizeText(message)
                            'OKCaption': context.localizeText('ok'),
                            'OnOK': '',
                        },
                    },
            ...
            {
                if(context.getPageProxy().binding.NotifNum=="")
                {
                    context.getPageProxy().binding.NotifNum= context.getPageProxy().binding.WOHeader.NotificationNumber
                }
                let noticationdetails=datacompletioncheck.getStepData(context,"notification")
                ////////////////////////////////
                //$expand=Items  Items M
                 context.read('/SAPAssetManager/Services/AssetManager.service', "Zmobilecmplt", [], '').then(Zmobilecmplt => {
                    console.log(Zmobilecmplt)
                })
                context.read('/SAPAssetManager/Services/AssetManager.service', "MyWorkOrderHeaders", [], '').then(woheader => {
                    console.log(woheader)
                })
                context.read('/SAPAssetManager/Services/AssetManager.service', "WorkCenters", [], '').then(WorkCenters => {
                    console.log(WorkCenters)
                })
                context.read('/SAPAssetManager/Services/AssetManager.service', "MyNotificationHeaders('" + context.getPageProxy().binding.NotifNum + "')", [], '$expand=Items/ItemCauses').then(results => {
                    if (results && results.length > 0) {
                        let Codegroup=-1;
                        
                        let objectpart=results.getItem(0).Items.findIndex((object)=>{ return object.DamageCode!=="" && object.CodeGroup!=="" && object.ObjectPartCodeGroup!=="" && object.ObjectPart!==""})
                        for (let index = 0; index < results.getItem(0).Items.length; index++) {
                            const element = results.getItem(0).Items[index];
                            let Codegrouphy=element.ItemCauses.findIndex((object)=>{ return object.CauseCode!=="" && object.CauseCodeGroup!==""})  
                             if(Codegrouphy==-1)  
                                {
                            ...
                            return context.executeAction(
                                {
                                    ...
                                    'Properties': {
                                        'Title': context.localizeText("completion_WO_title"),
                                        'Message': context.localizeText("Notification_malfunction_data_is_missing"),//"Notification malfunction data is missing",//context.localizeText(message)
                                        'OKCaption': context.localizeText('ok'),
                                        'OnOK': '',
                                    },
                                },
...
function CheckManditory(context,ordertype, plant,MaintenanceActivityType) {
    let filter = '';
    filter = "$filter=ZAUART eq '"+ordertype+"'";
    filter += " and ZWERKS eq '"+plant+"'";
    filter += " and ZILART eq '"+MaintenanceActivityType+"'";
    //alert(filter);
    return context.read('/SAPAssetManager/Services/AssetManager.service', 'Zmobilecmplt', ['ZWERKS','ZAUART','ZCATLOG','ZTECHOBJ','ZMSCAN'], filter).then(result => {
        if (result.length > 0) {
...
function CheckManditorynotification(context) {
    return context.read('/SAPAssetManager/Services/AssetManager.service', `MyNotificationHeaders('${context.binding.NotifNum}')`, [], '').then(result => {
        if (result.length > 0) {
            return result.getItem(0);
        }
        return undefined;
