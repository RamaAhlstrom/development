...
import { OperationControlLibrary as libOperationControl, PrivateMethodLibrary as libPrivate } from '../../../../SAPAssetManager/Rules/WorkOrders/Operations/WorkOrderOperationLibrary';
...
import { NoteLibrary } from '../../../../SAPAssetManager/Rules/Notes/NoteLibrary';
...
export class OperationLibrary {
        ...
        try {
            ...
            assignmentTypeValue.split(',').forEach(assignmentType => {
                ...
                switch (assignmentType) {
                    case '2':
                        filter = `(PersonNum eq '${libCommon.getPersonnelNumber()}' or sap.hasPendingChanges())`;
                        break;
                    case '4':
    ...
    static getOperationsFilterByAssgnTypeOrWCM(context) {
        return PersonaLibrary.isWCMOperator(context) ? OperationConstants.WCMOperationsFilter(context) : '';
    }
...
export async function isOperationSupportConfirmation(context, binding) {
    const isOperation = binding?.['@odata.type'] === '#sap_mobile.MyWorkOrderOperation' || binding?.['@odata.type'] === '#sap_mobile.MyWorkOrderSubOperation';

    if (isOperation && binding?.ControlKey) {
        const confirmationIsNotSupportedIndicator = '3';
        const confirmationIndicator = await context.read('/SAPAssetManager/Services/AssetManager.service', 'ControlKeys', ['ConfirmationIndicator'], `$filter=ControlKey eq '${binding.ControlKey}'`).then(result => result.length ? result.getItem(0).ConfirmationIndicator : null);

        return confirmationIndicator !== confirmationIsNotSupportedIndicator;
    }
