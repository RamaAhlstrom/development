...
export default async function WorkOrderOperationsListViewDefaultFilters(context) {
    ...
    if (myOperationListView === true) {
        let frd=await MyWorkSectionFilterQuery(context, '');
        const match = frd.match(/PersonNum eq '(\d+)'/);
            const number = match ? match[1] : null;
            console.log(number); // "01003394"
        //const myWorkFilter = "(PersonNum eq '01003394' or OperationMobileStatus_Nav/MobileStatus eq 'Started' or MyWorkOrderOperationCapacityRequirement_/any(mc : mc/PersonnelNo eq '01003394')) and OperationMobileStatus_Nav/MobileStatus ne 'Completed'";
          const myWorkFilter ="(PersonNum eq '"+number+"' or OperationMobileStatus_Nav/MobileStatus eq 'Started' or MyWorkOrderOperationCapacityRequirement_/any(mc : mc/PersonnelNo eq '"+number+"')) and OperationMobileStatus_Nav/MobileStatus ne 'Completed'"
