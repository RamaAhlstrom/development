...
export class NoteLibrary {
    ...
    static createUpdateOnPageLoad(pageClientAPI) {
        ...
        if (isOnRejectOp) {
           ...
           // noteTitle = pageClientAPI.localizeText('add_note');
           if(pageClientAPI.binding["@odata.id"].includes("MyWorkOrderHeaders"))
            {
                noteTitle = pageClientAPI.localizeText("add_note1");
            }
            else if(pageClientAPI.binding["@odata.id"].includes("MyWorkOrderOperations"))
            {
                noteTitle = pageClientAPI.localizeText("add_note1_op");
    
            }
            else if(pageClientAPI._page.previousPage.id.includes("PartDetailsPage")  || pageClientAPI.binding["@odata.id"].includes("MyWorkOrderComponents")  || pageClientAPI.binding["@odata.id"].includes("MyWorkOrderComponentLongTexts"))
            {
                noteTitle = pageClientAPI.localizeText("add_note1_part");
            }
            else if(pageClientAPI.binding["@odata.id"].includes("MyNotificationHeaders"))
            {
                noteTitle = pageClientAPI.localizeText("add_note_notification");
            }
            else
            {
                noteTitle = pageClientAPI.localizeText("add_note");
           ...
           if(pageClientAPI.binding["@odata.id"].includes("MyWorkOrderHeaders"))
             {
                 noteTitle = pageClientAPI.localizeText("edit_note1_wo");
             }
             else if(pageClientAPI.binding["@odata.id"].includes("MyWorkOrderOperations"))
             {
                 noteTitle = pageClientAPI.localizeText("edit_note1_op");
    
             }
             else if(pageClientAPI._page.previousPage.id.includes("PartDetailsPage")  || pageClientAPI.binding["@odata.id"].includes("MyWorkOrderComponents") || pageClientAPI.binding["@odata.id"].includes("MyWorkOrderComponentLongTexts"))
             {
                 noteTitle = pageClientAPI.localizeText("edit_note1_part");
             }
             else if(pageClientAPI.binding["@odata.id"].includes("MyNotificationHeaders"))
             {
                 noteTitle = pageClientAPI.localizeText("edit_note_notification");
             }
             else
             {
                 noteTitle = pageClientAPI.localizeText("edit_note");
...
if(pageClientAPI.getPageProxy()._page.id=="NoteAddWO")
        {
            noteTitle = pageClientAPI.localizeText("add_note1");
    ...
    static createLinks(context) {
       ...
       // return createLinks;
         if(libCommon.getStateVariable(context, 'longtexttype')== "Workorder"){
        //alert("empty")
        return [];

    }      else
    {
        //alert(JSON.stringify(createLinks))
          return createLinks;
    }
    }
