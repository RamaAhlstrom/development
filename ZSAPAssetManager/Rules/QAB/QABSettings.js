...
import CooperationIsEnabledForWorkOrder from '../../../SAPAssetManager/Rules/Cooperation/CooperationIsEnabledForWorkOrder';
...
import IsS4ServiceConfirmationEnabled from '../../../SAPAssetManager/Rules/ServiceConfirmations/IsS4ServiceConfirmationEnabled';
...
class QABSettingsHelpers {
    ...
    ////////////////////////////////////////////////
    async _createTakeReadingsChipG(props = {}) {
        let enable=true;
        let ge="/SAPAssetManager/Rules/Material/BOM/BOMPageNav.js"
        //&& await MeasuringPointsTakeReadingsIsVisible(this._context)
         return this._createChip({ ...{
            'Label': this._context.localizeText('bom'),
            ...
            'Action': '/SAPAssetManager/Rules/Material/BOM/BOMPageNav.js',
            '_Name': 'TAKE_BOM_READINGS',
        }, ...props});
    }
    /////////////////////////////////////////////////////////////////////
    ...
    _createAddNoteChip(/** @type {ChipProps?} */ props = {}) {
        ...
        let noteTitle="";
           if(this._context.getPageProxy().binding["@odata.id"].includes("WorkOrderH"))
            {
                noteTitle = this._context.localizeText("add_note1");
            }
            else if(this._context.getPageProxy().binding["@odata.id"].includes("Operations"))
            {
                noteTitle = this._context.localizeText("add_note1_op");
    
            }
            else if(this._context.getPageProxy().binding["@odata.id"].includes("Component"))
            {
                noteTitle = this._context.localizeText("add_note1_part");
            }
            else if(this._context.getPageProxy().binding["@odata.id"].includes("Notification"))
            {
                noteTitle = this._context.localizeText("add_note_notification");
            }
            else
            {
                noteTitle = this._context.localizeText("add_note");
            }
        return this._createChip({
            ...{
                'Label':noteTitle,//this._context.localizeText('add_note'),
    ...
    async _createCooperationSupportChip() {
        return this._createChip({
            ...
            'IsButtonVisible': CooperationIsEnabled(this._context),
            'IsButtonVisibleBySettings': false,
            'Action': '/SAPAssetManager/Rules/Cooperation/CooperationConfirmationNavWrapper.js',
    ...
    _createSDFChip() {
        const isSDFEnabled = SDFIsFeatureEnabled(this._context);
        return this._createChip({
