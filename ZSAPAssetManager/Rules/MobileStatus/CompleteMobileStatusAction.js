...
import IsInspectionLotEnabled from '../../../SAPAssetManager/Rules/WorkOrders/InspectionLot/IsInspectionLotEnabled';
import MobileStatusReviewUpdateLinks from '../../../SAPAssetManager/Rules/Supervisor/MobileStatus/MobileStatusReviewUpdateLinks';
        ...
        if (libCommon.isDefined(mobileStatusObject)) {
            ...
            return libSuper.checkReviewRequired(context, target).then(review => {
                ...
                return libClock.reloadUserTimeEntries(context, false, false, mobileInstance).then(() => {
                    return libClock.clockOutBusinessObject(context, mobileInstance).then(async () => {  ///async added for complete status 3673217.patch
                        ...
                        const acceptedStatus = await context.read('/SAPAssetManager/Services/AssetManager.service', 'EAMOverallStatusConfigs', ['EAMOverallStatus'], `$filter=MobileStatus eq '${completeMobileStatus}' and ObjectType eq '${eamObjType}' and EAMOverallStatusProfile eq '${mobileStatusObject.EAMOverallStatusProfile}'`);/////async added this line complete status 3673217.patch
                        const properties = {
                            ...
                            'EAMOverallStatus': acceptedStatus.getItem(0).EAMOverallStatus || '',/////async added this line complete status 3673217.patch
                        };
