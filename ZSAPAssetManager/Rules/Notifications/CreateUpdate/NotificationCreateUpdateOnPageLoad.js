...
export default function NotificationCreateUpdateOnPageLoad(context) {
    ...
    if (binding && binding['@odata.type'] === '#sap_mobile.InspectionCharacteristic') {
        ...
        if (onCreate) {
            ...
            /////////////////////////////////////////////////////////
            container.getControl('MainWorkCenterListPicker').setVisible(false);
            container.getControl('PlannerGroupListPicker').setVisible(false);
            container.getControl('BreakdownSwitch').setVisible(false);
            container.getControl('Persnal_Num').setVisible(false);
            try {                
               // effectControl.setValue(["3"]);
                container.getControl('EffectListPicker').setValue(["3"]);
            } catch (error) {
