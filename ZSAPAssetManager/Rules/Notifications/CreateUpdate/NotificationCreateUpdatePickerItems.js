...
import updateSoldToPartyPickers from '../../../../SAPAssetManager/Rules/Notifications/CreateUpdate/NotificationCreateUpdateSoldToPartyPickers';
        ...
        case 'EquipHierarchyExtensionControl':
            {
                ...
                if (equipmentControlValue) {
                    extensionPromise = context.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipments', ['FuncLocIdIntern','Location','WorkCenter_Main_Nav/ExternalWorkCenterId'], `$orderby=EquipId&$filter=EquipId eq '${equipmentControlValue}'&$expand=FunctionalLocation,WorkCenter_Main_Nav`).then( results => {
...
if(equipment!=undefined)
    {
        ...
        context.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipments', ['FuncLocIdIntern','Location'], `$filter=EquipId eq '${equipment}'&$expand=WorkCenter_Main_Nav`).then( results => {
            if (results.length > 0 && results.getItem(0).FuncLocIdIntern) {
                if(results.getItem(0).Location!="")
                {
                 if(formCellContainer.getControl('TechobjectLocation').getValue()=="" || formCellContainer.getControl('TechobjectLocation').getValue()==undefined)
                 {
                     formCellContainer.getControl('TechobjectLocation').setValue(results.getItem(0).Location)
                 }
                
                }
                
           //////////////////////////////
           if(results.getItem(0).WorkCenter_Main_Nav!=undefined)
           {
              // alert(results.getItem(0).WorkCenter_Main_Nav.ExternalWorkCenterId)
          formCellContainer.getControl('MainWorkCenterListPicker').setValue(results.getItem(0).WorkCenter_Main_Nav.ExternalWorkCenterId)
          formCellContainer.getControl('MainWorkCenterListPicker').setValue([results.getItem(0).WorkCenter_Main_Nav.ExternalWorkCenterId])
           }
          // if(results.getItem(0).FuncLocIdIntern!="")
       //         {
                  
       // context.binding["Techobjupdate"]=true;
       // formCellContainer.getControl('FuncLocHierarchyExtensionControl').clearValidation();
       // context.read('/SAPAssetManager/Services/AssetManager.service', 'MyFunctionalLocations', ['FuncLocIdIntern','Location','MaintWorkCenter'], `$filter=FuncLocIdIntern eq '${results.getItem(0).FuncLocIdIntern}'&$expand=WorkCenter_Main_Nav`).then( results => {
       //     if (results.length > 0 && results.getItem(0).FuncLocIdIntern) {
       //        console.log(results);
       //        if(results.getItem(0).Location!="")
       //        {
    ...
    {
        ResetValidationOnInput(context);
        context.read('/SAPAssetManager/Services/AssetManager.service', 'MyFunctionalLocations', ['FuncLocIdIntern','Location'], `$filter=FuncLocIdIntern eq '${floc}'&$expand=WorkCenter_Main_Nav`).then( results => {
            if (results.length > 0 && results.getItem(0).FuncLocIdIntern) {
               console.log(results);
               if(results.getItem(0).Location!="")
               {
    ...
    if((equipment==undefined && floc==undefined) || (equipment==undefined && floc=="") || (equipment=="" && floc==undefined))
        {
            formCellContainer.getControl('MainWorkCenterListPicker').setValue("");
        }
/////////////////////////////////////////////////////////////
